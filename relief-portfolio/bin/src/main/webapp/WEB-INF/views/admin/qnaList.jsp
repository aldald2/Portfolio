...
<body>
    ...
    <table class="noticeBoard">
        ...
        <tr onclick="selectQna(${ q.qid })">
            <td>${ q.qid }</td>
            <td>${ q.qtitle }</td>
            <td>${ q.aid }</td>
            <td>${ q.createDate }</td>
            <c:choose>
            	<c:when test="${ empty q.atitle }">
            		<td>X</td>
            	</c:when>
            	<c:otherwise>
            		<td>O</td>
    ...
    <div class="btnArea">
    <!-- 검색창 -->
    <form method="GET" action="${ contextPath }/admin/qSearch">
    ...
    <c:if test="${ pi.currentPage > 1 }">
    	...
    	<c:url var="before2" value="/admin/qSearch">
    		<c:param name="page" value="${ pi.currentPage - 1 }"/>
    ...
    <c:if test="${ pi.currentPage < pi.maxPage }">
    	...
    	<c:url var="after2" value="/admin/qSearch">
    		<c:param name="page" value="${ pi.currentPage + 1 }"/>
    		<c:param name="searchCondition" value="${ search.searchCondition }"/>
    		<c:param name="searchValue" value="${ search.searchValue }"/>
    	</c:url>
    ...
    <script>
    	function selectQna(qid){
    		location.href="${ contextPath }/admin/qnaDetail?qid=" + qid + "&page=${ pi.currentPage }";
    	}
    </script>
</body>
</html>